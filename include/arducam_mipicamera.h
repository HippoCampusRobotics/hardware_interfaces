// this is a copy of ArduCAM's MIPI camera SDK and the original file can be
// found here: https://github.com/ArduCAM/MIPI_Camera
 ...
 * Timestamps in MMAL are defined as signed 64 bits integer values representing
 * microseconds. However a pre-defined special value is used to signal that a
 * timestamp is not known. */
/* @{ */
#define TIME_UNKNOWN \
  (INT64_C(1) << 63) /**< Special value signalling that time is not known */
/* @} */
...
typedef struct {
  ...
  int quantisationParameter;  /// Quantisation parameter - quality. Set bitrate
                              /// 0 and set this for variable bitrate
...
typedef struct {
  ...
  uint32_t length;     /**< Number of bytes currently used in the payload buffer
...
struct camera_interface {
  ...
  int sda_pins[2];  // enable sda_pins[camera_num], disable sda_pins[camera_num
                    // ? 0 : 1]
  int scl_pins[2];  // enable scl_pins[camera_num], disable scl_pins[camera_num
                    // ? 0 : 1]
  int shutdown_pins[2];
  int led_pins[2];
};

// note The buffer will be automatically released after the callback function
// ends.
...
/**
 * init camera.
 * @param camera_instance Pointer of type CAMERA_INSTANCE, use to obtain
 CAMERA_INSTANCE.
 ...
 * */
int arducam_init_camera2(CAMERA_INSTANCE *camera_instance,
                         struct camera_interface cam_interface);

/**
 * Set output resolution.
 * @param camera_instance Type CAMERA_INSTANCE, Obtained from
 arducam_init_camera function.
 * @param width Pointer of type int, Used to specify the width and return to the
 actual width.
 * @param height Pointer of type int, Used to specify the height and return to
 the actual height.
 * @return error code , 0 success, !0 error.
 * example:
 @code
 ...
 @note Some boards have multiple camera interfaces.
 * */
int arducam_set_resolution(CAMERA_INSTANCE camera_instance, int *width,
                           int *height);
 ...
 * @param camera_instance Type CAMERA_INSTANCE, Obtained from
 * arducam_init_camera function.
 * @param mode Mode index.(You can use the list_format program to view the
 * supported modes.)
 * @return error code , 0 success, !0 error.
 */
 ...
 * @return error code , 0 success, !0 error.
 */
int arducam_set_lens_table(CAMERA_INSTANCE camera_instance,
                           char *lens_table_path);
 ...
 * @param camera_instance Type CAMERA_INSTANCE, Obtained from
 arducam_init_camera function.
 * @param encoder_state Used to specify encoding parameters. Use default
 parameters if NULL.
 * @param callback Callback method, this method will be called when there is
 data return.
 * @param userdata Userdata, which will be a member of the buffer parameter in
 ...
 * example:
 ...
 @endcode
 @note Calling the arducam_set_resolution function before stopping video
 encoding will terminate the video encoding.
 @note The buffer will be automatically released after the callback function
 ends.
 * */
int arducam_set_video_callback(CAMERA_INSTANCE camera_instance,
                               VIDEO_ENCODER_STATE *encoder_state,
                               OUTPUT_CALLBACK callback, void *userdata);
 ...
 * example:
 ...
 @endcode
 @note If you do a time-consuming operation in the callback, it will affect
 other parts, such as preview, video encoding(This issue may be fixed in the
 future).
 @note The buffer will be automatically released after the callback function
 ends.
 * */
int arducam_set_raw_callback(CAMERA_INSTANCE camera_instance,
                             OUTPUT_CALLBACK callback, void *userdata);
 ...
 * example:
 ...
 ends.
 * */
int arducam_set_yuv_callback(CAMERA_INSTANCE camera_instance,
                             OUTPUT_CALLBACK callback, void *userdata);
 ...
 * @param timeout This method will return NULL if no data is obtained at this
 time.
 ...
 * example:
 ...
 @note When the buffer is used, you need to call the arducam_release_buffer
 function to release it.
 @note The actual width and height of the raw bayer format and the yuv420 format
 are aligned, width 32 bytes aligned, and height 16 byte aligned.
 * */
BUFFER *arducam_capture(CAMERA_INSTANCE camera_instance, IMAGE_FORMAT *format,
                        int timeout);
/**
 ...
 * @return error code , 0 success, !0 error.
 * */
int arducam_start_preview(CAMERA_INSTANCE camera_instance,
 ...
 * @return error code , 0 success, !0 error.
 * */
int arducam_start_preview_fix_lens(CAMERA_INSTANCE camera_instance,
                                   PREVIEW_PARAMS *preview_params,
                                   char *lens_table_path);
/**
 ...
 * example:
 ...
 @endcode
 * */
int arducam_set_control(CAMERA_INSTANCE camera_instance, int ctrl_id,
                        int value);
 ...
 * example:
 ...
 @endcode
 * */
int arducam_get_control(CAMERA_INSTANCE camera_instance, int ctrl_id,
                        int *value);
 ...
 * example:
 ...
 @endcode
 * */
int arducam_get_support_formats(CAMERA_INSTANCE camera_instance,
                                struct format *fmt, int index);
 ...
 * example:
 @code
    ...
    while (!arducam_get_support_controls(camera_instance, &support_cam_ctrl,
 index++)) { int value = 0; if (arducam_get_control(camera_instance,
 support_cam_ctrl.id, &value)) { printf("Get ctrl %s fail.\n",
 support_cam_ctrl.desc);
        }
        printf("index: %d, CID: 0x%08X, desc: %s, min: %d, max: %d, default: %d,
 current: %d\n", index, support_cam_ctrl.id, support_cam_ctrl.desc,
 support_cam_ctrl.min_value, support_cam_ctrl.max_value,
 support_cam_ctrl.default_value, value);
    }
 @endcode
 * */
int arducam_get_support_controls(CAMERA_INSTANCE camera_instance,
                                 struct camera_ctrl *cam_ctrl, int index);
 ...
 * @note Calling the arducam_set_resolution function will turn off this feature.
 */
int arducam_software_auto_white_balance(CAMERA_INSTANCE camera_instance,
                                        int enable);
 ...
 * @return BUFFER structure pointer containing image data.
 *
 * @note This function will remove the part that is filled because of the
 * alignment,
 * @note for example, the height is 1080, because the height needs 16 is the
 * alignment,
 * @note so the actual pixel height is 1088. After passing the height of 1080
 * using this
...
// void arducam_manual_set_awb_compensation(CAMERA_INSTANCE camera_instance,
// uint32_t r_gain, uint32_t b_gain);
